...
arch=('x86_64' 'aarch64')
url="https://github.com/Firstp1ck/packman"
license=('MIT')
depends=('gcc-libs')
optdepends=(
  'python: pip support'
...
fetch_source() {
  cd "$srcdir" || exit 1
  if [ ! -d packman ]; then
    git clone --filter=blob:none --sparse https://github.com/Firstp1ck/packman.git packman
  fi
  cd packman || exit 1
...
pkgver() {
  if [ ! -d "$srcdir/packman" ]; then
    fetch_source >/dev/null 2>&1
  fi
  cd "$srcdir/packman" || exit 1
...
package() {
  cd "$srcdir/packman" || exit 1
  install -Dm755 "target/release/packman" "$pkgdir/usr/bin/packman"
